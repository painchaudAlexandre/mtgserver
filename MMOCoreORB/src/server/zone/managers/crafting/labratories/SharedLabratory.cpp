...
int SharedLabratory::calculateAssemblySuccess(CreatureObject* player,DraftSchematic* draftSchematic, float effectiveness){
	...
	printf("%s: assemblySkill=%d\d",__FILE__, player->getSkillMod(draftSchematic->getAssemblySkill());
	assemblySkill += player->getSkillMod("force_assembly");
	printf("%s: forceAssemblySkill=%d\d", __FILE__, player->getSkillMod(draftSchematic->getAssemblySkill());
